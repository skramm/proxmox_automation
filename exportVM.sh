...
function process()
{
	curl -k -H 'Authorization: PVEAPIToken=krammseb@UR!'"${TOK_NAME}=${TOK_VALUE}" \
		https://anvers.univ-rouen.fr:8006/api2/json/nodes/$1/qemu>>out_$1.json 2>>curl_log.log
...
# liste des nodes 
	curl -k -H "Authorization: PVEAPIToken=krammseb@UR!${TOK_NAME}=${TOK_VALUE}" \
		https://anvers.univ-rouen.fr:8006/api2/json/nodes|jq | jq -r '.data[] | .node' >nodes.csv 2>>curl_log.log
